...
export const Heading1EditModal = ({
  openChange,
  session,
  id,
  blockId,
  ...
  const [content, setContent] = useState<heading1>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      ...
      try {
        const response = await fetch(`/api/heading1/${id}`, {
          method: "GET",
          headers: {
        ...
        const data: heading1 = await response.json();
        setContent(data);
        setIsFetching(false);
      } catch (error) {
